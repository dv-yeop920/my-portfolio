  ...
  projects: [
    {
      ...
      team: "4인 팀 프로젝트",
      contribution: "프론트엔드 개발 · 기획 일부 참여",
      role: [
    ...
    },
    {
      ...
      role: [
        "서비스 기획부터 프론트엔드·백엔드 개발, 운영까지 전반 담당",
        "일정 공지 → 신청 → 배정 → 확정으로 이어지는 사용자 흐름 설계 및 구현",
      ...
      highlights: [
        {
          ...
          description:
            "Harness Engineering 기반으로 **프로젝트 규칙과 작업 범위를 먼저 정의하고 조사 → 계획 → 승인 → 구현 → 리뷰 단계와 Agent 역할을 분리**했습니다. 기획 의도와 아키텍처를 AI에 명확히 전달하고 단계별 결과를 검토하며 구현 범위 이탈과 수정 비용을 줄였습니다.",
        },
        {
          title: "Supabase 기반 데이터 흐름 설계",
          description:
            "**회원 정보를 기준으로 일정 신청 → 인원 배정 → 근무 확정 → 급여 계산 데이터가 이어지도록 Supabase DB 구조를 설계했습니다.** 일정마다 달라지는 근무 정보는 회원 데이터와 연결하고 확정된 근무시간과 시급·수당 정보를 급여 계산에 다시 활용하도록 구성했습니다.",
        },
        {
          title: "Zod 기반 입력 데이터 검증",
          description:
            "회원가입과 일정·급여 입력 등 **검증이 필요한 기능마다 Zod 스키마를 분리해 입력 조건과 예외를 정의했습니다.** 화면에서 전달되는 데이터를 스키마 기준으로 먼저 검증한 뒤 처리하도록 구성해 잘못된 값이 이후 로직으로 전달되는 경우를 줄였습니다.",
        },
      ],
      retrospective:
      ...
      flow: [
        {
          ...
          solution:
            "내부 구성원만 가입할 수 있도록 Supabase에 저장된 가입 코드의 활성 상태와 사용 가능 횟수를 확인한 뒤 회원가입을 진행하도록 구성했습니다. 또한 잘못된 데이터가 저장되는 것을 방지하기 위해 이름·연락처 등 필수 입력값을 검증해 유효한 코드와 정보가 확인된 경우에만 계정을 생성하도록 회원가입 흐름을 설계했습니다.",
          images: [
            {
        ...
        },
        {
          ...
          solution:
            "기존에는 카카오톡 신청 내용을 메모장에 옮겨 날짜별 근무 가능 인원을 다시 정리하고 이전 스케줄을 찾아보며 포지션 배치까지 직접 맞춰야 했습니다. 이를 날짜별 신청자와 기존 배정 정보를 한 화면에서 확인하고 바로 포지션을 배정·수정할 수 있도록 구성해 흩어진 정보를 찾고 비교하는 과정을 줄였습니다.",
          images: [
            {
        ...
        },
        {
          ...
          solution:
            "기존에는 근무시간과 수당을 직접 계산해 급여를 확인했고 실제 지급액과 차이가 생기면 다시 계산해야 했습니다. 확정된 근무 일정과 시급·수당을 기준으로 급여를 자동 계산하고 주·월 단위로 내역을 확인할 수 있게 해 수기 계산과 금액을 다시 대조하는 과정을 줄였습니다.",
          images: [
            {
        ...
        },
        {
          ...
          solution:
            "가입 코드를 운영 상황에 맞게 관리할 수 있도록 Supabase에 코드별 사용 가능 횟수와 활성 상태를 저장하는 구조를 설계했습니다. 관리자는 각 코드의 상태를 조회·수정하고 사용 횟수가 소진된 경우 신규 코드를 발급하거나 갱신할 수 있도록 구현해 회원가입에 사용하는 코드를 데이터 기준으로 관리할 수 있게 했습니다.",
          images: [
            {
        ...
        },
        {
          ...
          solution:
            "중요 공지 사항을 채팅방에 따로 알리면 중요한 내용이 대화에 묻히기 쉽기 때문에 관리자가 필요한 내용을 공지로 등록하고 대상 근무자에게 전달할 수 있게 해 일정 관련 안내를 서비스 안에서 확인하도록 했습니다.",
          images: [
            {
